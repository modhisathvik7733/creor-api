...
export type ModelId = keyof typeof GATEWAY_MODELS

/** Get per-1K-token cost for a model. Returns fallback if model not found. */
export function getModelCost(model: string): { input: number; output: number } {
  const entry = GATEWAY_MODELS[model as ModelId]
